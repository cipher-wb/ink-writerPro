...
import ink_writer.core.extract.style_sampler as sampler_module
from ink_writer.core.extract.style_sampler import StyleSampler, StyleSample, SceneType
...
def test_style_sampler_rejects_ai_smell_sample(temp_project):
    sampler = StyleSampler(temp_project)
    bad_text = "与此同时，他不由得深吸一口气。与此同时，他不由得深吸一口气。与此同时，他不由得深吸一口气。"
    sample = StyleSample(
        id="bad_ai",
        chapter=2,
        scene_type=SceneType.DESCRIPTION.value,
        content=bad_text,
        score=0.95,
        tags=["模板"],
    )
    assert sampler.add_sample(sample) is False
    assert sampler.lint_text(bad_text)["passed"] is False
