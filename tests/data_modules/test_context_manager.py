...
from ink_writer.core.state.snapshot_manager import SnapshotManager, SnapshotVersionMismatch
from ink_writer.core.context.query_router import QueryRouter
...
def test_context_manager_build_and_filter(temp_project):
    ...
    (temp_project.ink_dir / "project_memory.json").write_text(json.dumps({"patterns": []}, ensure_ascii=False), encoding="utf-8")
...
def test_context_manager_includes_structured_memory_sections(temp_project):
    ...
    idx.save_chapter_memory_card(
        ChapterMemoryCardMeta(
            chapter=2,
            summary="药老没有解释戒指异动。",
            next_chapter_bridge="第3章必须接住戒指与药老的关系",
    ...
    assert memory["previous_chapter_memory_card"]["chapter"] == 2
    assert memory["active_plot_threads"][0]["thread_id"] == "ring-secret"
    assert memory["recent_timeline_anchors"][0]["chapter"] == 2
    assert any(row["entity_id"] == "xiaoyan" for row in memory["related_entity_state_changes"])
