...
class TestBackupIndexDb:
    ...
    def test_backup_index_db_success(self, manager_git, monkeypatch):
        ...
        with patch.dict("sys.modules", {
            "ink_writer.core.infra.config": MagicMock(DataModulesConfig=MagicMock(from_project_root=MagicMock(return_value=fake_cfg))),
            "ink_writer.core.index.index_manager": MagicMock(IndexManager=MagicMock(return_value=fake_mgr_instance)),
        }):
    ...
    def test_backup_index_db_exception_is_swallowed(self, manager_git):
        ...
        with patch.dict("sys.modules", {
            "ink_writer.core.infra.config": broken,
            "ink_writer.core.index.index_manager": MagicMock(),
        }):
            # Should not raise
