...
# ---------------------------------------------------------------------------
# patch_outline_hook_contract (backfill script)
# ---------------------------------------------------------------------------

class TestPatchOutlineHookContract:
    def test_patch_missing_contract(self, tmp_path):
        from patch_outline_hook_contract import patch_chapter_block
        patched, modified = patch_chapter_block(MISSING_CONTRACT_BLOCK, 7, 20)
        assert modified is True
        contract = extract_hook_contract_from_outline(patched)
        assert contract is not None
        assert contract.hook_type == "crisis"
        assert contract.anchor_chapter == 8

    def test_skip_existing_contract(self, tmp_path):
        from patch_outline_hook_contract import patch_chapter_block
        patched, modified = patch_chapter_block(VALID_CHAPTER_BLOCK, 3, 20)
        assert modified is False
        assert patched == VALID_CHAPTER_BLOCK

    def test_patch_project(self, tmp_path):
        from patch_outline_hook_contract import patch_project
        outline_dir = tmp_path / "大纲"
        outline_dir.mkdir()
        outline_file = outline_dir / "第1卷-详细大纲.md"
        outline_file.write_text(TWO_CHAPTER_VOLUME, encoding="utf-8")

        results = patch_project(tmp_path)
        assert results["files_scanned"] == 1
        assert results["chapters_patched"] == 1

        patched_content = outline_file.read_text(encoding="utf-8")
        assert "钩子契约" in patched_content
        errors = validate_volume_outline(patched_content)
        assert len(errors) == 0
    ...
    def test_dry_run_no_modification(self, tmp_path):
        ...
        results = patch_project(tmp_path, dry_run=True)
        assert results["chapters_patched"] == 1

        content = outline_file.read_text(encoding="utf-8")
        assert content == TWO_CHAPTER_VOLUME
    ...
    def test_backup_created(self, tmp_path):
        ...
        backup = outline_dir / "第1卷-详细大纲.md.bak.pre_hook_contract"
        assert backup.exists()
        assert backup.read_text(encoding="utf-8") == TWO_CHAPTER_VOLUME

    def test_guess_hook_types(self):
        from patch_outline_hook_contract import guess_hook_type
        assert guess_hook_type("悬念钩 - 身份之谜") == "mystery"
        assert guess_hook_type("危机钩 - 敌人来袭") == "crisis"
        assert guess_hook_type("情绪钩 - 感人离别") == "emotion"
        assert guess_hook_type("选择钩 - 两难抉择") == "choice"
        assert guess_hook_type("渴望钩 - 追求力量") == "desire"
        assert guess_hook_type("未知类型的钩子") == "mystery"
