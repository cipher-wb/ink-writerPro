...
[![Version](https://img.shields.io/badge/Version-10.5.0-green.svg)](ink-writer/.claude-plugin/plugin.json)
...
| 版本 | 说明 |
|------|------|
| **v10.5.0 (当前)** | 爽点质量提升+完结检测 — 爽点链规划+压扬标记+信息差设计+Craft6场景1500字+题材基线注入+3重完结拦截 |
| **v10.4.0** | 写作质量约束升级 — 禁止时间标记开头 + 元数据泄漏清理 + ink-fix回溯修复 + 标题去重 + benchmark动态加载 |
| **v10.3.0** | ink-fix skill 自动修复 + 编码校验 + 约束注入闭环 |
| **v10.0.0** | 工程架构审查优化 — 异常处理收紧 + SQLite WAL模式 + 魔法数字收口 + ruff linter + 日志级别提升 |
