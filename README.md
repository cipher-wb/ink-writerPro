...
[![Version](https://img.shields.io/badge/Version-8.2.0-green.svg)](ink-writer/.claude-plugin/plugin.json)
...
日常用 `/ink-auto` 就够了。但如果你想要更精细的控制，这些姿势也可以单独使用：

```bash
# ═══ 从零开始 ═══
/ink-init              # 开局：创建小说项目，像第一次约会一样交互式填写设定
/ink-plan 1            # 规划：生成第 1 卷详细大纲（节拍表+时间线+章纲）

# ═══ 日常写作（二选一）═══
/ink-auto 5            # 【推荐】全自动：写 5 章 + 自动审查修复 + 自动规划
/ink-5                 # 单会话写 5 章 + Full 审查（适合大上下文模型）

# ═══ 单章操作 ═══
/ink-write             # 写一章：完整 9 步流水线，2200 字起步
/ink-review 1-5        # 审查：10 个 Agent 仔细检查第 1-5 章
/ink-resume            # 续命：中断了？从断点恢复
...
| | `/ink-5` | `/ink-auto` |
|--|----------|-------------|
| **过程** | 同一会话，一气呵成 | 每章独立会话，做完一次换个新的 |
| **适合** | 大上下文模型（1M+）| 小上下文模型（200k）或任何模型 |
| **审查** | 5 章写完后统一 Full 审查 | 每 5 章自动 Core 审查 + 修复 |
| **智能程度** | 基础 | 内置检查点 + 自动大纲 + 自动修复 |
| **持久力** | 5 章后需要休息 | 100 章不喊停就不停 |

**结论：无脑选 `/ink-auto`。** 它比 `/ink-5` 更持久、更智能、更省心。
...
启动 4 个核心审查 Agent（consistency + continuity + OOC + anti-detection），发现 critical/high 问题**立即自动修复**，修复后自动验证。

覆盖：设定矛盾、连贯断裂、人设崩塌、AI 味过浓、伏笔遗漏。
...
| 版本 | 亮点 |
|------|------|
| **v8.2.0 (当前)** | 角色知识门控（Knowledge Gate）：主角视角知识边界约束 + 第四定律 + CKV 检查；爽点前置密集化：章内三段布局 + 开场速度约束 + 密度基线强化 |
