...
├─────────────────────────────────────────────────────────────┤
│  Agents (35个, M4+): Context / Writer / Polish / Data       │
│    + Writer-self-check (M3) + 28 Checkers (含 M3 新 2 个    │
...
US-006 起直白化链路全场景激活，避免文字冗余、读起来费劲；黄金三章与战斗/高潮/爽点仍作为重点阈值桶处理。v22
引入场景感知直白化信号：context-agent 装填 `pack["meta"]["scene_mode"]`（由
...
`directness-checker`（7 维度 0-10 评分 D1 修辞密度 / D2 形容词-动词比 / D3 抽象词密度 /
D4 句长适中 / D5 空描写段 / D6 嵌套深度 / D7 修饰链长）按 `reports/seed_thresholds.yaml` 跑阈值 → polish-agent
`## Simplification Pass` 精简（消费 `ink-writer/assets/prose-blacklist.yaml` 107 条黑名单
+ 长句拆分 + 修辞压缩 + 70% 字数下限回滚保护）。冲突解耦：writer-agent L10b/L10e 与
sensory-immersion-checker 在全场景 Directness Mode 暂挂；prose-impact-checker /
...
writer/checker/polish 共享。M-6 验证锁定 sensory-immersion 残余输出在全场景下不会反向触发 Red。
