...
3. `PROJECT_ROOT`（真实小说项目根，`/ink-init` 按书名创建）
...
```text
project-root/
├── .ink/            # 运行时数据（state/index/vectors/summaries）
├── 正文/                  # 正文章节
├── 大纲/                  # 总纲与卷纲
└── 设定集/                # 世界观、角色、力量体系
```
...
```text
${CLAUDE_HOME:-~/.claude}/ink-writer/workspaces.json
```
...
- 指针文件：`D:\wk\novel skill\plugin-sim-20260303-012048\.claude\.ink-current-project`
...
```bash
python "${SCRIPTS_DIR}/ink.py" --project-root "${PROJECT_ROOT}" index process-chapter --chapter 1
python "${SCRIPTS_DIR}/ink.py" --project-root "${PROJECT_ROOT}" index stats
```

### 健康报告

```bash
python "${SCRIPTS_DIR}/ink.py" --project-root "${PROJECT_ROOT}" status -- --focus all
...
```bash
python "${SCRIPTS_DIR}/ink.py" --project-root "${PROJECT_ROOT}" rag index-chapter --chapter 1
python "${SCRIPTS_DIR}/ink.py" --project-root "${PROJECT_ROOT}" rag stats
```
