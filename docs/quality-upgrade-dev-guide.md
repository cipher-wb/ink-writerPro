...
### Phase 3: 商业包装 + 黄金三章
- [x] T3.1 — 商业包装系统（commercial-packaging.md：书名/简介/题材指南）
- [x] T3.2 — 黄金三章checker升级（10秒扫读测试+配角检查+里程碑参考）
- [x] T3.3 — 审核模式大纲约束（ink-plan增加前3万字里程碑约束）
- [x] T3.4 — 开篇模式库（golden-opening-patterns.md：5种成功模式+5种禁忌）
- [ ] T3.5 — **QG3: 10秒扫读测试**（待端到端验证时一并测试）

### Phase 4: 反AI味根治
- [x] T4.1 — 重写anti-detection-writing.md（从机械规则→内化风格原则，7条硬规则→7条风格原则+AI词表）
- [x] T4.2 — AI高频词黑名单（ai-word-blacklist.md）
- [ ] T4.3 — anti-detection-checker升级（留待后续迭代）
- [ ] T4.4 — polish-agent升级（留待后续迭代）
- [ ] T4.5 — **QG4: AI检测+人工盲审**（待端到端验证）

### Phase 5: 角色声音系统
- [x] T5.1 — 角色卡schema扩展（speech_profile定义在ooc-checker中）
- [x] T5.2 — ooc-checker升级（新增第5.5步声音区分度检查）
- [ ] T5.3 — **QG5: 角色区分度测试**（待端到端验证）
