...
def create_app(project_root: str | Path | None = None) -> FastAPI:
    ...
    async def _lifespan(_: FastAPI):
        ink = _ink_dir()
        if ink.is_dir():
            _watcher.start(ink, asyncio.get_running_loop())
        try:
            yield
        finally:
            _watcher.stop()

    app = FastAPI(title="Ink Dashboard", version="9.7.0", lifespan=_lifespan)
    ...
    def project_info():
        ...
        state_path = _ink_dir() / "state.json"
    ...
    @app.get("/api/chapter-memory-cards")
    def list_chapter_memory_cards(limit: int = 50):
        with closing(_get_db()) as conn:
    ...
    def list_plot_threads(status: Optional[str] = None, limit: int = 100):
        with closing(_get_db()) as conn:
            if status:
                return _fetchall_safe(
                    conn,
                    "SELECT * FROM plot_thread_registry WHERE status = ? ORDER BY priority DESC, last_touched_chapter DESC LIMIT ?",
                    (status, limit),
                )
            return _fetchall_safe(
                conn,
                "SELECT * FROM plot_thread_registry ORDER BY priority DESC, last_touched_chapter DESC LIMIT ?",
                (limit,),
            )
    ...
    @app.get("/api/candidate-facts")
    def list_candidate_facts(entity: Optional[str] = None, limit: int = 100):
        with closing(_get_db()) as conn:
            if entity:
                return _fetchall_safe(
                    conn,
                    "SELECT * FROM candidate_facts WHERE entity_id = ? ORDER BY chapter DESC, confidence DESC LIMIT ?",
                    (entity, limit),
                )
            return _fetchall_safe(
                conn,
                "SELECT * FROM candidate_facts ORDER BY chapter DESC, confidence DESC LIMIT ?",
                (limit,),
            )
    ...
    @app.get("/api/events")
    async def sse():
        """Server-Sent Events 端点，推送 .ink/ 下的文件变更。"""
    ...
    if STATIC_DIR.is_dir():
        ...
        def no_frontend():
            return HTMLResponse(
                "<h2>Ink Dashboard API is running</h2>"
