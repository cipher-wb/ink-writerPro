...
def _resolve_project_root(cli_root: str | None) -> Path:
    ...
    env = os.environ.get("INK_PROJECT_ROOT")
    if env:
        return Path(env).resolve()

    # 尝试从 .claude 指针读取
    cwd = Path.cwd()
    pointer = cwd / ".claude" / ".ink-current-project"
    if pointer.is_file():
        target = pointer.read_text(encoding="utf-8").strip()
        if target:
            p = Path(target)
            if p.is_dir() and (p / ".ink" / "state.json").is_file():
                return p.resolve()

    # 最终兜底：当前目录
    if (cwd / ".ink" / "state.json").is_file():
        return cwd.resolve()

    print("ERROR: 无法定位 PROJECT_ROOT（需要包含 .ink/state.json 的目录）", file=sys.stderr)
    sys.exit(1)


def main():
    parser = argparse.ArgumentParser(description="Ink Dashboard Server")
