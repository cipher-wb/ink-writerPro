...
监控 PROJECT_ROOT/.ink/ 目录下 state.json / index.db 等文件的写事件，
...
class _InkFileHandler(FileSystemEventHandler):
    """仅关注 .ink/ 目录下关键文件的修改/创建事件。"""
...
class FileWatcher:
    ...
    def start(self, watch_dir: Path, loop: asyncio.AbstractEventLoop):
        ...
        self._loop = loop
        handler = _InkFileHandler(self._on_change)
