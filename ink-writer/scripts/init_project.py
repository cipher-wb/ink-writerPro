...
目标：
- 生成可运行的项目结构（ink-project）
- 创建/更新 .ink/state.json（运行时真相）
- 生成基础设定集与大纲模板文件（供 /ink-plan 与 /ink-write 使用）

说明：
- 该脚本是命令 /ink-init 的“唯一允许的文件生成入口”（与命令文档保持一致）。
...
def _ensure_state_schema(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    - state.json 保持精简 (< 5KB)
    """
    state.setdefault("schema_version", 9)
...
def _build_master_outline(target_chapters: int, *, chapters_per_volume: int = _DEFAULT_CPV) -> str:
    ...
    lines: list[str] = [
        "# 总纲",
        "",
        "> 本文件为“总纲骨架”，用于 /ink-plan 细化为卷大纲与章纲。",
        "",
        "## 卷结构",
        "",
    ]
...
def init_project(
    ...
    *,
    ...
    # state.json（创建或增量补齐）
    state_path = project_path / ".ink" / "state.json"
...
# Don't ignore .ink (we need to track state.json)
# But ignore cache files
.ink/context_cache.json
.ink/*.lock
.ink/*.bak
""",
                        encoding="utf-8",
...
def main() -> None:
    ...
    parser.add_argument("project_dir", help="项目目录（建议 ./ink-project）")
