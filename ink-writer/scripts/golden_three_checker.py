...
class GoldenThreeChecker:
    ...
    def __init__(self, chapter_files: List[str], mode: str = "keyword", project_root: Optional[str] = None):
        """
        初始化检查器
        ...
        Args:
            ...
            project_root: 项目根目录（用于从 state.json 加载主角名）
        """
        ...
        self._protagonist_keywords = self._load_protagonist_keywords()
    ...
    def _load_protagonist_keywords(self) -> list:
        """从 state.json 动态加载主角名，保留通用关键词作为 fallback。"""
        generic = ["我", "主角", "少年", "他", "她"]
        if not self.project_root:
            return generic
        state_file = self.project_root / ".ink" / "state.json"
        if not state_file.exists():
            return generic
        try:
            state = json.loads(state_file.read_text(encoding="utf-8"))
            name = state.get("protagonist_state", {}).get("name", "")
            if name:
                # 添加主角名和可能的姓/名拆分
                names = [name]
                if len(name) >= 2:
                    names.append(name[0])   # 姓
                    names.append(name[1:])  # 名
    ...
    def check_chapter1_keywords(self) -> None:
        ...
        # 检查1: 主角 300 字内出场
        for keyword in self._protagonist_keywords:
...
""".strip(),
    ...
    try:
        checker = GoldenThreeChecker(chapter_files, mode=args.mode, project_root=args.project_root)
        checker.run()
