...
def test_workflow_lifecycle_and_trace(tmp_path, monkeypatch):
    ...
    module.start_task("ink-write", {"chapter_num": 7})
...
def test_start_task_reentry_increments_retry(tmp_path, monkeypatch):
    ...
    module.start_task("ink-write", {"chapter_num": 8})
    module.start_task("ink-write", {"chapter_num": 8})
...
def test_complete_step_rejects_mismatch_step_id(tmp_path, monkeypatch):
    ...
    module.start_task("ink-write", {"chapter_num": 9})
...
def test_workflow_step_owner_and_order_violation_trace(tmp_path, monkeypatch):
    ...
    assert module.expected_step_owner("ink-write", "Step 1") == "context-agent"
    assert module.expected_step_owner("ink-write", "Step 5") == "data-agent"

    module.start_task("ink-write", {"chapter_num": 12})
    module.start_step("Step 3", "Review")
...
def test_get_workflow_paths_support_zero_arg_find_project_root(tmp_path, monkeypatch):
    ...
    assert module.get_workflow_state_path() == tmp_path / ".ink" / "workflow_state.json"
    assert module.get_call_trace_path() == tmp_path / ".ink" / "observability" / "call_trace.jsonl"
...
def test_workflow_reentry_does_not_duplicate_history(tmp_path, monkeypatch):
    ...
    module.start_task("ink-write", {"chapter_num": 20})
    module.start_task("ink-write", {"chapter_num": 20})
    module.start_task("ink-write", {"chapter_num": 20})
