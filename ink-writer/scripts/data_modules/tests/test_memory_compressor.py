...
class TestCheckCompressionNeeded:
    def test_early_chapter_no_compression(self, tmp_path):
        """50 章以内不需要压缩"""
        from data_modules.memory_compressor import check_compression_needed
        # 创建最小项目结构
...
class TestLoadVolumeSummaries:
    def test_load_volume_summaries(self, tmp_path):
        from data_modules.memory_compressor import load_volume_summaries
        ink_dir = tmp_path / ".ink"
        summaries_dir = ink_dir / "summaries"
...
class TestBuildMegaSummaryPrompt:
    def test_prompt_structure(self, tmp_path):
        from data_modules.memory_compressor import build_mega_summary_prompt
        summaries = [
...
class TestSaveMegaSummary:
    def test_save_creates_file(self, tmp_path):
        from data_modules.memory_compressor import save_mega_summary
        ink_dir = tmp_path / ".ink"
        summaries_dir = ink_dir / "summaries"
        summaries_dir.mkdir(parents=True)
...
class TestDefaultChaptersPerVolumeEnvVar:
    def test_env_var_override(self, monkeypatch):
        ...
        # 需要重新导入以获取新的环境变量
        import importlib
        import data_modules.memory_compressor as mc
        importlib.reload(mc)
