...
class IndexObservabilityMixin:
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._get_conn() as conn:
            ...
            cursor.execute("SELECT COUNT(*) FROM chapter_memory_cards")
            chapter_memory_cards = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM plot_thread_registry")
            plot_thread_registry = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM timeline_anchors")
            timeline_anchors = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM candidate_facts")
            candidate_facts = cursor.fetchone()[0]
            ...
            return {
                ...
                "chapter_memory_cards": chapter_memory_cards,
                "plot_thread_registry": plot_thread_registry,
