...
if $PYTHON_LAUNCHER -c 'import ink_writer.debug.cli' >/dev/null 2>&1; then
  exec $PYTHON_LAUNCHER -m ink_writer.debug.cli --project-root "$PROJECT" status "$@"
fi
exec env PYTHONPATH="$SHIM_DIR:${PYTHONPATH:-}" $PYTHON_LAUNCHER -m ink_writer.debug.cli --project-root "$PROJECT" status "$@"
