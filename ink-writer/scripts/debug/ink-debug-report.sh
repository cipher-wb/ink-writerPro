...
if python3 -c 'import ink_writer.debug.cli' >/dev/null 2>&1; then
  exec python3 -m ink_writer.debug.cli --project-root "$PROJECT" report "$@"
fi
exec env PYTHONPATH="$SHIM_DIR:${PYTHONPATH:-}" python3 -m ink_writer.debug.cli --project-root "$PROJECT" report "$@"
