...
$env:TMP = $tmpRoot
$env:TEMP = $tmpRoot
$env:PYTHONPATH = "ink-writer/scripts"
...
try:
    d = Path(tempfile.mkdtemp(prefix="ink_writer_pytest_"))
...
if ($Mode -eq "smoke") {
    python -m pytest -q `
        ink-writer/scripts/data_modules/tests/test_extract_chapter_context.py `
        ink-writer/scripts/data_modules/tests/test_rag_adapter.py `
        --basetemp $baseTemp `
        --no-cov `
        -p no:cacheprovider
    exit $LASTEXITCODE
...
python -m pytest -q `
    ink-writer/scripts/data_modules/tests `
    --basetemp $baseTemp `
    -p no:cacheprovider
exit $LASTEXITCODE
