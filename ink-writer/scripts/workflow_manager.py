...
def find_project_root(override: Optional[Path] = None) -> Path:
    """Resolve project root (containing .ink/state.json).
...
def get_workflow_state_path() -> Path:
    ...
    return project_root / ".ink" / "workflow_state.json"
...
def expected_step_owner(command: str, step_id: str) -> str:
    ...
    """
    if command == "ink-write":
        mapping = {
            "Step 1": "context-agent",
            "Step 1.5": "ink-write-skill",
        ...
        return mapping.get(step_id, "ink-write-skill")

    if command == "ink-review":
        return "ink-review-skill"
    ...
    if step_id == "Step 3":
        return [
            ...
            },
            {
                ...
                "label": "跳过审查直接润色",
                "risk": "low",
                "description": "后续可用 /ink-review 补审",
...
def _backup_chapter_for_cleanup(project_root: Path, chapter_num: int, chapter_path: Path) -> Path:
    ...
    backup_dir = project_root / ".ink" / "recovery_backups"
