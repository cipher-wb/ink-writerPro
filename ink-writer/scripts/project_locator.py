#!/usr/bin/env python3
"""
Project location helpers for ink-writer scripts.

Problem this solves:
- Many scripts assumed CWD is the project root and used relative paths like `.ink/state.json`.
- In this repo, commands/scripts are often invoked from the repo root, while the actual project lives
  in a subdirectory (default: `ink-project/`).
...
DEFAULT_PROJECT_DIR_NAMES: tuple[str, ...] = ("ink-project",)
CURRENT_PROJECT_POINTER_REL: Path = Path(".claude") / ".ink-current-project"

# 用户级全局映射（当 skills/agents 安装在 ~/.claude 时，项目目录可能在任意盘符）
# 该文件用于在“空上下文 + CWD 不在项目内”的情况下仍能定位到正确 project_root。
GLOBAL_REGISTRY_REL: Path = Path("ink-writer") / "workspaces.json"

# Claude Code 常见环境变量（存在时优先作为“工作区根目录”提示）
ENV_CLAUDE_PROJECT_DIR = "CLAUDE_PROJECT_DIR"
ENV_CLAUDE_HOME = "CLAUDE_HOME"
ENV_INK_CLAUDE_HOME = "INK_CLAUDE_HOME"
...
def _get_user_claude_root() -> Path:
    ...
    if raw:
        try:
            ...
            logger.warning("failed to resolve claude home path %s", raw, exc_info=True)
...
def _load_global_registry(path: Path) -> dict:
    ...
    try:
        ...
        logger.warning("failed to load global registry %s", path, exc_info=True)
        return _default_registry()
...
def _save_global_registry(path: Path, data: dict) -> None:
    ...
    try:
        ...
        logger.warning("failed to save global registry %s", path, exc_info=True)
        return
...
def update_global_registry_current_project(
    *,
    ...
    try:
        ...
        logger.warning("failed to resolve project root %s", root, exc_info=True)
    ...
    try:
        ...
        logger.warning("failed to resolve workspace root %s", ws, exc_info=True)
        ws = ws.expanduser()
...
def _is_project_root(path: Path) -> bool:
    return (path / ".ink" / "state.json").is_file()
...
def write_current_project_pointer(project_root: Path, *, workspace_root: Optional[Path] = None) -> Optional[Path]:
    ...
    if ws_root is not None:
        ...
        if (ws_root / ".claude").is_dir():
            try:
                ...
                logger.warning("failed to write project pointer file", exc_info=True)
                pointer_file = None
    ...
    try:
        ...
        logger.warning("failed to update global registry", exc_info=True)
...
def resolve_project_root(explicit_project_root: Optional[str] = None, *, cwd: Optional[Path] = None) -> Path:
    """
    Resolve the ink project root directory (the directory containing `.ink/state.json`).

    Resolution order:
    1) explicit_project_root (if provided)
    2) env var INK_PROJECT_ROOT (if set)
    3) Search from cwd and parents, including common subdir `ink-project/`
    ...
    if explicit_project_root:
        ...
        # 兼容：显式传入“工作区根目录”（含 `.claude/.ink-current-project` 指针）
        ...
        # 显式传入的目录默认视为用户指定的项目根；
        # 允许初始化前或仅存在 `.ink/` 子目录的场景继续执行，由下游命令自行决定是否需要 state.json。
        if root.is_dir():
            return root

        raise FileNotFoundError(f"Not an ink project root (missing .ink/state.json): {root}")

    env_root = os.environ.get("INK_PROJECT_ROOT")
    if env_root:
        ...
        if root.is_dir():
            return root
        raise FileNotFoundError(f"INK_PROJECT_ROOT is set but invalid (missing .ink/state.json): {root}")
    ...
    raise FileNotFoundError(
        "Unable to locate ink project root. Expected `.ink/state.json` under the current directory, "
        "a parent directory, or `ink-project/`. Run /ink-init first or pass --project-root / set "
        "INK_PROJECT_ROOT."
    )
