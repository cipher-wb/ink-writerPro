...
除 Strand Weave 平衡外，额外检测章节内部的紧凑度：

### 事件密度检测

每500字至少应有1个可识别的**推进动作**（以下任一）：
- 角色做出决策或行动
- 获取新信息/线索
- 关系发生变化（结盟/冲突/分离）
- 局势发生转变
...
| 指标 | 阈值 | 严重度 |
|------|------|--------|
| 连续500字无推进动作 | 事件密度过低 | `medium` |
| 连续800字无推进动作 | 严重空转 | `high` |

### 空转检测

**空转定义**：连续300字以上的纯描写/纯回忆/纯解释，且不包含任何新信息（读者在前文已知的重复信息）。

| 指标 | 阈值 | 严重度 |
|------|------|--------|
| 存在300字以上空转段 | 节奏拖沓 | `medium` |
| 存在500字以上空转段 | 严重拖沓 | `high` |

### 内心独白比例检测

**计算方式**：统计全章中"角色内心独白/心理分析"占总字数的比例。

| 指标 | 阈值 | 严重度 |
|------|------|--------|
| 内心独白比例 > 30% | 独白偏多 | `medium` |
| 内心独白比例 > 50% | 严重过度内心化 | `high` |

**修复建议**：
- 空转段 → 删除或融入对话/行动中
- 内心独白过多 → 用"做"代替"想"，用行动和对话展现角色心理
- 事件密度不足 → 插入微推进（一个小决策、一条新信息、一个关系微变化）
