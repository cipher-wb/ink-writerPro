...
| HARD-005 | 开篇空洞 | 前500字无冲突/风险/强情绪/悬念 | high |

**HARD-005 开篇空洞检测**：
- 扫描章节前500字（约占2200字章节的23%）
- 检测是否存在以下至少一项：冲突、风险/危险、强情绪（愤怒/恐惧/震惊/心痛）、悬念/信息缺口
- 纯环境描写/纯背景交代/纯日常流水账不计入
- 过渡章可放宽到前800字
...
检测开头模式时，显式识别以下类型：
- **时间标记开头**：含"X月X日"、"那天"、"次日"、"翌日"、"清晨/黄昏/夜里"等直接时间陈述作为第一句
- **环境描写开头**：以景物/天气/氛围描写起笔
- **对话开头**：以角色对话直接切入
- **行动开头**：以角色动作或事件切入
- **感官/内心开头**：以角色感知或心理活动切入
- **承接开头**：直接回应上章钩子

其中"时间标记开头"为高风险类型，连续2章即触发 warning，连续3章触发 risk。
