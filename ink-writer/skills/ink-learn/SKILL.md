...
## 执行流程
1. 读取 `"$PROJECT_ROOT/.ink/state.json"`，获取当前章节号（progress.current_chapter）
2. 读取 `"$PROJECT_ROOT/.ink/project_memory.json"`，若不存在则初始化 `{"patterns": []}`
