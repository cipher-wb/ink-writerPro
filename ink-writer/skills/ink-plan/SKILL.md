...
若 exit code = 0（同步成功/无待同步），跳至 §7.3。
若 exit code = 1（BLOCKER/冲突），按 §7.2 冲突处理执行。
若脚本不存在，回退到 §7.2 手动模式。
...
**7. 设定补全检查**
- sync_settings.py（§7.1）已执行且 exit code = 0（若脚本不存在，以 §7.2 手动模式为准）
- 本卷涉及的新角色/势力/规则已回写到现有设定集文件
- 所有新增条目可回溯到本卷章纲章节
- `BLOCKER` 数量为 0；若 sync_settings.py exit code = 1（冲突）或手动审查发现冲突 >0，必须先裁决，不得进入 state 更新
