...
- **爽点前置原则**（hard requirements）：
  - 卷级爽点链 PC-1 的爆发章不得晚于本卷第 3 章（确保开卷即有爆发，不允许"前5章全铺垫"）
...
**第 1 章完整小闭环（硬约束）**：
第 1 章大纲必须额外包含以下必填字段：
- **第1章闭环**: `能力展示={事件} → 危机={事件} → 能力收益={具体结果}`
  - **能力展示**：主角的金手指/核心能力首次被读者看到的事件（不能只是内心感知）
  - **危机**：因能力暴露、环境突变、或外部威胁而产生的紧迫危机
  - **能力收益**：主角通过使用能力获得的具体、可感知的收益
  - 三个子字段任一为空 → hard fail
  - `能力收益` 必须是具体的、可感知的变化（如”获得报酬””地位提升””新能力解锁””危机解除后的奖励””救下关键人物”），**不能是抽象描述**（如”主角有了新认知””主角理解了能力的意义”）→ hard fail
  - 第 1 章的 `章末未闭合问题` 必须指向主线大冲突（不能还停留在”主角在想这个能力是什么”这种探索层级）→ hard fail
...
**1.7 第1章完整小闭环检查**
- 第 1 章大纲必须包含 `第1章闭环` 字段 → 缺失则 hard fail
- `第1章闭环` 的三个子字段（能力展示/危机/能力收益）任一为空 → hard fail
- `能力收益` 不能是抽象描述（含"认知""理解""意识到""感悟""明白了"等关键词）→ hard fail
...
- 第 1 章额外必须有：第1章闭环（能力展示 + 危机 + 能力收益，三项均非空）
...
- 每章包含：目标/阻力/代价/时间锚点/章内时间跨度/与上章时间差/大卖点类型/大卖点描述/小卖点(×2)/爽点执行/压扬标记/Strand/反派层级/视角/关键实体/主角行动/本章变化/章末未闭合问题/钩子/钩子契约/伏笔处置/明暗线推进
...
- **时间线表文件不存在或为空**
- 章纲文件不存在或为空
- 任一章节缺少：目标/阻力/代价/时间锚点/章内时间跨度/与上章时间差/大卖点类型/大卖点描述/小卖点(×2)/爽点执行/压扬标记/Strand/反派层级/视角/关键实体/主角行动/本章变化/章末未闭合问题/钩子/钩子契约/伏笔处置/明暗线推进
...
- **第 1 章缺少 `第1章闭环` 字段**（第1章完整小闭环违反）
- **第 1 章闭环三子字段（能力展示/危机/能力收益）任一为空**（第1章完整小闭环违反）
- **第 1 章 `能力收益` 为抽象描述（含认知/理解/意识到/感悟/明白了）**（第1章完整小闭环违反）
- **第 1 章 `章末未闭合问题` 停留在探索层级而非指向主线大冲突**（第1章完整小闭环违反）
