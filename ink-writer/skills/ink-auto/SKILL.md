...
| 触发条件 | 操作 | 预估耗时 |
|----------|------|----------|
| **每5章** (5,10,15...) | ink-review Core（最近5章）+ 自动修复 critical/high 问题 | ~15-20min |
| **每10章** (10,20,30...) | + ink-audit quick（数据健康检查） | +2min |
| **每20章** (20,40,60...) | + ink-audit standard + ink-macro-review Tier2 + 消歧积压检查 | +25min |
...
审查发现 critical/high 级别问题时自动修复：
- 修复后自动重审验证（最多1轮，防止无限循环）
- medium/low 级别问题仅记录在审查报告中
- 所有审查报告保存在 `审查报告/` 目录
