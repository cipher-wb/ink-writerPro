...
**每个检查点操作都包含自动修复**，不只是审查：

- **审查修复**：ink-review 内置修复（Step 7 选项A） + 独立修复进程二次补刀
- **审计修复**：读取审计报告，修复 state.json/index.db 数据不一致、过期伏笔标记、幽灵实体
- **宏观审查修复**：读取宏观报告，修复支线停滞标记、承诺追踪更新、风格漂移校正

修复流程：检查报告 → 检测是否含 critical/high/严重问题 → 无问题跳过 → 有问题启动独立 CLI 修复进程 → git commit
