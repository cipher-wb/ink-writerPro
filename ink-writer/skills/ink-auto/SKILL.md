...
2. **大纲覆盖预检**：逐章检查所有 N 章的大纲是否存在
   - 使用 `extract-context --chapter {ch} --format pack` 逐章验证
   - **任意一章缺少大纲 → 整个批次立即中止，不写入任何章节**
   - 输出所有缺失大纲的章节号，提示用户执行 `/ink-plan`
...
2. **逐章大纲二次检查**（防止预检后大纲被外部删除）
3. 清理上一轮的 workflow 残留状态（`workflow clear`）
4. 启动全新 CLI 进程执行完整 ink-write 流程
5. 等待进程退出（阻塞调用，保证串行）
6. **检查进程退出码**：非零时报告可能的网络/超时/崩溃原因
7. 冷却 10 秒（确保 git/index 异步操作完成）
8. 多重验证：章节文件 + 字数 ≥ 2200 + state 更新 + 摘要
9. 验证失败则用 ink-resume 重试一次
10. 仍失败则中止，输出详细错误报告（已完成章数、失败章节、可能原因、日志路径）
...
- **大纲强校验**：批量开始前 + 每章写作前双重检查，无大纲则立即中止
- **异常必报告**：任何失败（网络、超时、崩溃）都会输出详细诊断信息到终端和日志
