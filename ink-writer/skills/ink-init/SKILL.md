...
3 套方案**统一**按 Quick Step 0.5 锁定的档位生成（不做梯度分离）。档位映射到以下硬参数，生成前必须读入并据此约束每套方案：

| 档位 | 名称 | 元规则命中下限/套 | 扰动对/套 | 种子稀有度门槛 | 粗口密度 | 附加硬约束 |
|------|------|-------------------|-----------|----------------|----------|------------|
| 1 | 保守 | ≥1 | 1 | rarity ≤3 | 0%（L1 禁） | V3→V1 回退 |
| 2 | 平衡 | ≥2 | 2 | 至少 1 对含 rarity 4 | ≈0.2% | — |
| 3 | 激进 | ≥3 | 3 | 至少 2 对含 rarity 4 | 0.5%-0.8%（L2 仅 V3） | — |
| 4 | 疯批 | ≥5 | 5 | 至少 3 对含 rarity 5 | 0.8%-1.5%（L2 仅 V3） | **每套方案必须打破 ≥1 商业安全边界**；至少 1 套 V3 |

> 以上表与 Quick Step 0.5 档位矩阵同源；变更须同步 `meta-creativity-rules.md / perturbation-engine.md / style-voice-levels.md`。
