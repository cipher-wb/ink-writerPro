...
- `reader-pull-checker`
- `flow-naturalness-checker`（US-014 新增，核心文笔层——信息节奏/融入/过渡/对话辨识/黄金比例/语气/voice 一致）
...
- `prose-impact-checker`（US-014 新增，文笔冲击力——镜头/感官/句式/动词锐度/环境共振/特写缺失）
- `sensory-immersion-checker`（US-014 新增，感官沉浸深度——主导感官轮换/深度/通感/情绪匹配/抽象替代）
- `reader-simulator`
...
- `prose-impact-checker`（US-014）：当满足任一条件时启用
  - `chapter <= 3`（黄金三章强制启用）；
  - 关键章/高潮章/卷末章（需要冲击力保障）；
  - 正文出现战斗、反杀、打脸、情绪高光等高密度冲击信号；
  - 用户显式要求"文笔冲击力审查"/"镜头审查"。
- `sensory-immersion-checker`（US-014）：当满足任一条件时启用
  - `chapter <= 3`（黄金三章强制启用）；
  - 情感章/悬疑章/战斗章（感官沉浸价值高）；
  - 用户显式要求"感官审查"/"沉浸审查"。
...
| `flow-naturalness-checker` | `chapter_text`、`outline`、`scene_context`、`voice_profile`、`info_budget` | 信息节奏/对话辨识/voice 一致（US-014）|
...
| `prose-impact-checker` | `proofreading-checker` | 同属文本表层+文笔层（US-014）|
| `sensory-immersion-checker` | `proofreading-checker` | 感官描写属文本表层（US-014）|
...
selected = [“consistency-checker”, “continuity-checker”, “ooc-checker”,
            ...
            “flow-naturalness-checker”]  # US-014: 核心自然流畅度
...
if mode != “minimal”:
  ...
  # US-014: 文笔冲击+感官沉浸（ch1-3 强制 + 关键章/情感章/战斗章/悬疑章命中）
  if chapter <= 3 or trigger_prose_impact:
      selected.append(“prose-impact-checker”)
  if chapter <= 3 or trigger_sensory_immersion:
      selected.append(“sensory-immersion-checker”)
...
| `flow-naturalness-checker` | 5% | 自然流畅度 | 核心（US-014）——信息节奏/对话辨识/voice 一致 |
| `prose-impact-checker` | 5% | 文笔冲击力 | 条件（US-014）——镜头/感官丰富/句式/动词锐度/环境共振 |
| `sensory-immersion-checker` | 3% | 感官沉浸 | 条件（US-014）——主导感官轮换/深度/通感/抽象替代 |
...
- **主角能动性门禁**：若 `high-point-checker` 报告 `CAMERA_PROTAGONIST` severity=critical（全章无主角主动行为），禁止进入 Step 4，必须回退 Step 2A 重写。黄金三章中 `PASSIVE_STREAK` severity=high 也升级为 hard block。摄像头主角是结构性问题，润色无法补救。
- **人类本能反应门禁**：若 `ooc-checker` 报告 `NO_RESPONSE_TO_INJURY` severity=critical（场景 A/C 发生但在场角色完全无反应），禁止进入 Step 4，必须回退 Step 2A 重写。黄金三章中 `FLAT_TO_LIFE_THREAT` / `NO_EMOTION_ON_WINDFALL` severity=high 升级为 hard block。无反应的触发场景是人物失真，润色无法补救。
- **文笔冲击力门禁**（US-014）：若 `prose-impact-checker` 存在任一维度 `severity = critical` 或 ch1-3 任一维度 `severity = high`，禁止进入 Step 4，必须回退 Step 2A 重写（见下方文笔冲击力门禁规则）。`SHOT_MONOTONE.CHAPTER`（全章单一镜头类型 > 60%）和 `CAMERA_CLOSEUP_MISSING`（关键冲突点无特写）是 hard block 信号。
- **自然流畅度门禁**（US-014）：若 `flow-naturalness-checker` 存在 `INFO_OVERLOAD.CHAPTER` / `DIALOGUE_INDISTINGUISHABLE` / `NARRATION_AS_INFO_DUMP` 任一 `critical`，或 ch1-3 存在 `VOICE_PROFILE_VIOLATION` / `INFO_OVERLOAD.SEGMENT` `high`，禁止进入 Step 4，必须回退 Step 2A 重写（见下方自然流畅度门禁规则）。这是"塞信息感"与"对话雷同"的结构性门禁。
- **文笔工艺门禁**：若 `proofreading-checker` Layer 6 报告 `WEAK_VERB_OVERUSE.TOTAL`（全章弱动词 >15 次）或 `SENSORY_DESERT.CHAPTER`（全章非视觉感官 <3 处）severity=critical，禁止进入 Step 4，必须回退 Step 2A 重写。黄金三章中 `WEAK_VERB_OVERUSE.SINGLE`（单词 >3 次）与 `SENSORY_DESERT.SEGMENT`（连续 800+ 字无非视觉感官）severity=high 升级为 hard block。文笔工艺是章节质感的底线，critical 级问题需重写段落而非仅依赖润色兜底（见下方文笔工艺门禁规则）。
...
### 主角能动性门禁（Protagonist Agency Gate）

> US-010 新增。确保审查层拦截摄像头主角问题。

**Hard Block（回退 Step 2A 重写）**：
- `high-point-checker` 报告 `CAMERA_PROTAGONIST` severity = `critical`（全章主角无主动改变局面的行为）
- 黄金三章（ch1-3）中 `PASSIVE_STREAK` severity = `high`（主角连续800+字被动）→ 升级为 hard block

**Soft Warning（传递给 polish-agent 修复）**：
- 普通章节 `PASSIVE_STREAK` severity = `high` → 进入 Step 4 润色修复
- `NO_CONSEQUENCE` severity = `high`（行动无后果）→ 进入 Step 4 润色修复
- `STATIC_SITUATION` severity = `high`（处境未变）→ 进入 Step 4 润色修复
- `STATIC_SITUATION` severity = `medium`（悬念过渡章豁免降级）→ 建议修复

**闸门判定逻辑**：
```text
agency_issues = high-point-checker.issues.filter(rule in ["CAMERA_PROTAGONIST", "PASSIVE_STREAK", "NO_CONSEQUENCE", "STATIC_SITUATION"])
is_golden_three = chapter <= 3

camera_critical = filter(agency_issues, rule="CAMERA_PROTAGONIST", severity="critical")
passive_high = filter(agency_issues, rule="PASSIVE_STREAK", severity="high")

if len(camera_critical) > 0:
    BLOCK: "主角能动性门禁触发：全章无主角主动行为（CAMERA_PROTAGONIST）"
    repair_context = [{
        "issue_type": "CAMERA_PROTAGONIST",
        "severity": "critical",
        "location": "全章",
        "fix_suggestion": "主角必须做出至少1个改变局面的主动行为，产生可观察后果"
    }]
    return BLOCKED, repair_context

if is_golden_three and len(passive_high) > 0:
    BLOCK: "主角能动性门禁触发（黄金三章加严）：主角连续800+字被动（PASSIVE_STREAK）"
    repair_context = [{
        "issue_type": "PASSIVE_STREAK",
        "severity": "high→hard_block(golden_three)",
        "location": issue.location,
        "fix_suggestion": "在被动长段中插入主角主动行为，打破观察/思考/感受的连续段"
    } for issue in passive_high]
    return BLOCKED, repair_context

# 普通章节 PASSIVE_STREAK / NO_CONSEQUENCE / STATIC_SITUATION → 传递 polish 修复
agency_high = filter(agency_issues, severity="high")
if len(agency_high) > 0:
    通过（带修复建议）: "主角能动性有 {len(agency_high)} 个 high 问题，转 polish 修复"
else:
    通过: "主角能动性检查通过"
```

### 人类本能反应门禁（Human Instinct Gate）

> US-012 新增。与 writer-agent L9 人类本能反应律（US-011）配对，确保审查层拦截角色反应失真。

**Hard Block（回退 Step 2A 重写）**：
- `ooc-checker` 报告 `NO_RESPONSE_TO_INJURY` severity = `critical`（场景 A 受伤/倒下或场景 C 极端事件发生，在场角色 200 字内完全无反应关键词）
- 黄金三章（ch1-3）中 `FLAT_TO_LIFE_THREAT` severity = `high`（主角面对生命威胁平静无波动）→ 升级为 hard block
- 黄金三章（ch1-3）中 `NO_EMOTION_ON_WINDFALL` severity = `high`（主角获重大好处毫无情绪）→ 升级为 hard block
- 黄金三章（ch1-3）中 `INSULT_NO_REACTION` severity = `high`（被侮辱后继续谈笑风生）→ 升级为 hard block

**Soft Warning（传递给 polish-agent 修复）**：
- 普通章节 `FLAT_TO_LIFE_THREAT` / `NO_EMOTION_ON_WINDFALL` / `INSULT_NO_REACTION` severity = `high` → 进入 Step 4 润色修复
- `BYSTANDER_SILENCE` severity = `medium`（围观群众失声）→ Step 4 润色补救
- `UNDERSIZED_REACTION` severity = `medium`（反应段字数不足）→ Step 4 润色补充（黄金三章升级为 high）
- `UNREASONABLE_NPC` severity = `medium`（路人配角反应与场景严重度错位）→ Step 4 润色修复
- `CONTRACT_MISS_TRIGGER` severity = `medium`（Contract 未预声明触发场景）→ 记录反馈，下章 Contract 修正
- 豁免标签降级后的 `low`（冷血/超脱等设定集支撑）→ 建议微润色补微表情/微动作

**闸门判定逻辑**：
```text
hi_issues = ooc-checker.issues.filter(rule startswith "NO_RESPONSE" or rule in
    ["FLAT_TO_LIFE_THREAT", "NO_EMOTION_ON_WINDFALL", "INSULT_NO_REACTION",
     "BYSTANDER_SILENCE", "UNDERSIZED_REACTION", "UNREASONABLE_NPC", "CONTRACT_MISS_TRIGGER"])
is_golden_three = chapter <= 3

no_response_critical = filter(hi_issues, rule="NO_RESPONSE_TO_INJURY", severity="critical")
gt_flat_threat = filter(hi_issues, rule="FLAT_TO_LIFE_THREAT", severity="high") if is_golden_three else []
gt_flat_windfall = filter(hi_issues, rule="NO_EMOTION_ON_WINDFALL", severity="high") if is_golden_three else []
gt_insult_noreact = filter(hi_issues, rule="INSULT_NO_REACTION", severity="high") if is_golden_three else []

blockers = no_response_critical + gt_flat_threat + gt_flat_windfall + gt_insult_noreact

if len(blockers) > 0:
    BLOCK: "人类本能反应门禁触发：{len(blockers)} 个人物失真问题"
    repair_context = [{
        "issue_type": issue.rule,
        "severity": issue.severity + ("→hard_block(golden_three)" if is_golden_three and issue.severity=="high" else ""),
        "location": issue.location,
        "fix_suggestion": issue.fix_hint or "补充触发场景对应的反应关键词（动作/感官/情绪/思维）+ 达到字数最低要求"
    } for issue in blockers]
    return BLOCKED, repair_context
else:
    hi_high = filter(hi_issues, severity="high")
    if len(hi_high) > 0:
        通过（带修复建议）: "人类本能反应有 {len(hi_high)} 个 high 问题，转 polish 修复"
    else:
        通过: "人类本能反应检查通过"
```

### 文笔工艺门禁（Prose Craft Gate）

> US-014 新增。与 writer-agent L10 遣词造句律（US-013）配对，确保审查层拦截弱动词泛滥、感官沙漠等文笔质感问题。判据见 `skills/ink-write/references/prose-craft-rules.md`。

**Hard Block（回退 Step 2A 重写）**：
- `proofreading-checker` Layer 6 报告 `WEAK_VERB_OVERUSE.TOTAL` severity = `critical`（全章弱动词累计 > 15 次）
- `proofreading-checker` Layer 6 报告 `SENSORY_DESERT.CHAPTER` severity = `critical`（全章非视觉感官 < 3 处）
- 黄金三章（ch1-3）中 `WEAK_VERB_OVERUSE.SINGLE` severity = `high`（单词 > 3 次）→ 升级为 hard block
- 黄金三章（ch1-3）中 `SENSORY_DESERT.SEGMENT` severity = `high`（连续 800+ 字无非视觉感官）→ 升级为 hard block

**Soft Warning（传递给 polish-agent Layer 8 修复）**：
- 普通章节 `WEAK_VERB_OVERUSE.SINGLE` severity = `high` → Step 4 polish-agent Layer 8a 弱动词替换
- 普通章节 `SENSORY_DESERT.SEGMENT` severity = `high` → Step 4 polish-agent Layer 8b 感官锚点补充
- `ADJECTIVE_PILE` severity = `medium` → Step 4 polish-agent Layer 8c 形容词精简
- `GENERIC_DESCRIPTION` severity = `medium`（单处）或 `high`（累计 ≥5 处）→ Step 4 polish-agent Layer 8c 空洞表达改写
- `ADJECTIVE_PILE` severity = `low`（2 个修饰语均为空洞形容词）→ 建议微润色

**闸门判定逻辑**：
```text
pc_issues = proofreading-checker.issues.filter(rule in [
    "WEAK_VERB_OVERUSE.SINGLE", "WEAK_VERB_OVERUSE.TOTAL",
    "SENSORY_DESERT.SEGMENT", "SENSORY_DESERT.CHAPTER",
    "ADJECTIVE_PILE", "GENERIC_DESCRIPTION", "GENERIC_DESCRIPTION.DENSITY"
])
is_golden_three = chapter <= 3

pc_critical = filter(pc_issues, severity="critical")
gt_weak_high = filter(pc_issues, rule="WEAK_VERB_OVERUSE.SINGLE", severity="high") if is_golden_three else []
gt_sensory_high = filter(pc_issues, rule="SENSORY_DESERT.SEGMENT", severity="high") if is_golden_three else []

blockers = pc_critical + gt_weak_high + gt_sensory_high

if len(blockers) > 0:
    BLOCK: "文笔工艺门禁触发：{len(blockers)} 个结构性文笔问题"
    repair_context = [{
        "issue_type": issue.rule,
        "severity": issue.severity + ("→hard_block(golden_three)" if is_golden_three and issue.severity=="high" else ""),
        "location": issue.location or "全章",
        "fix_suggestion": issue.fix_suggestion or "参考 prose-craft-rules.md 对应章节示例库进行重写"
    } for issue in blockers]
    return BLOCKED, repair_context
else:
    pc_high = filter(pc_issues, severity="high")
    if len(pc_high) > 0:
        通过（带修复建议）: "文笔工艺有 {len(pc_high)} 个 high 问题，转 polish Layer 8 修复"
    else:
        通过: "文笔工艺检查通过"
```

**回退次数限制**：
- 文笔工艺门禁独立计数回退次数，与逻辑/大纲合规/卖点密度/主角能动性/人类本能反应门禁各自独立
- 最多触发 **2 次**回退；第 3 次失败 → 暂停流程请求人工干预（复用"逻辑/合规门禁回退机制"的暂停输出模板）
- `repair_context` 膨胀控制：只传 critical + golden_three 升级的 high issue 精简信息，总 token 预算 ≤ 500
...
### 文笔冲击力门禁（Prose Impact Gate）

> US-014 新增。基于 `prose-impact-checker` 六维评分（镜头多样性/感官丰富度/句式节奏/动词锐度/环境-情绪共振/特写缺失）判定文笔冲击力是否过关。与文笔工艺门禁（proofreading Layer 6）互补——后者查单词/单段微观问题，本门禁查"场景—镜头—句式"宏观冲击力缺失。

**Hard Block（回退 Step 2A 重写）**：
- 任一维度 `score = D` 且 severity = `critical`
- 黄金三章（ch1-3）中任一维度 `score <= C` 且 severity = `high` → 升级为 hard block
- `SHOT_MONOTONE.CHAPTER`（全章单一镜头类型占比 > 60%）severity = `critical`
- `CAMERA_CLOSEUP_MISSING`（关键情绪/冲突点无特写）severity = `critical`

**Soft Warning（传递给 polish-agent Layer 9 修复）**：
- 普通章节任一维度 severity = `high` → Step 4 polish Layer 9a/9b/9d
- `SENTENCE_CV_LOW`（句长 CV < 0.40）severity = `high` → Step 4 polish Layer 9d
- `WEAK_VERB_ACTION`（动作场景弱动词 > 30%）severity = `high` → Step 4 polish Layer 9 协同 Layer 8a
- `ENV_EMOTION_MISMATCH` severity = `high` → Step 4 polish Layer 9

**闸门判定逻辑**：
```text
pi_issues = prose-impact-checker.issues
pi_critical = filter(pi_issues, severity="critical")
gt_high = filter(pi_issues, severity="high") if chapter <= 3 else []

blockers = pi_critical + gt_high

if len(blockers) > 0:
    BLOCK: "文笔冲击力门禁触发：{len(blockers)} 个 critical/黄金三章 high 问题"
    repair_context = [{
        "issue_type": issue.rule,
        "severity": issue.severity + ("→hard_block(golden_three)" if chapter<=3 and issue.severity=="high" else ""),
        "location": issue.location or "全章",
        "fix_suggestion": issue.fix_suggestion or "参考 L10d/L10e/L10f/L10g 重写镜头与句式"
    } for issue in blockers]
    return BLOCKED, repair_context
else:
    pi_high = filter(pi_issues, severity="high")
    if len(pi_high) > 0:
        通过（带修复建议）: "文笔冲击力有 {len(pi_high)} 个 high 问题，转 polish Layer 9 修复"
    else:
        通过: "文笔冲击力检查通过"
```

**回退次数限制**：最多 2 次；第 3 次同章触发 → 暂停流程请求人工干预（复用通用暂停模板）。

### 自然流畅度门禁（Flow Naturalness Gate）

> US-014 新增。基于 `flow-naturalness-checker` 七维评分（信息密度均匀/融入方式/过渡/对话辨识/黄金比例/语气一致/voice 一致）判定"不自然感"与"塞信息感"。**核心 checker**，始终执行。

**Hard Block（回退 Step 2A 重写）**：
- `INFO_OVERLOAD.CHAPTER`（全章信息密度严重超标：单段 >2 个新概念 + 超出 `info_budget.max_new_concepts`）severity = `critical`
- `DIALOGUE_INDISTINGUISHABLE`（去掉角色名后所有对话无法区分说话人）severity = `critical`
- `NARRATION_AS_INFO_DUMP`（纯叙述性设定解释占比 > 60%）severity = `critical`
- 黄金三章（ch1-3）中 `VOICE_PROFILE_VIOLATION`（主角对话严重违背 voice_profile）severity = `high` → 升级为 hard block
- 黄金三章（ch1-3）中 `INFO_OVERLOAD.SEGMENT` severity = `high` → 升级为 hard block

**Soft Warning（传递给 polish-agent Layer 9c/9d 修复）**：
- `INFO_OVERLOAD.SEGMENT` severity = `high` → Step 4 polish Layer 9c 信息稀释
- `DIALOGUE_GOLDEN_RATIO_VIOLATION`（对话40-50%:叙述30-40%:心理10-20% 偏离 ±10%）severity = `high` → Step 4 polish
- `TRANSITION_ABRUPT`（视角/时空跳跃无过渡）severity = `high` → Step 4 polish
- `TONE_INCONSISTENT`（叙述者语气在不正式/口语/文学间不自然切换）severity = `high` → Step 4 polish
- `VOICE_PROFILE_VIOLATION` severity = `medium`（配角偶发违背）→ 建议修复

**闸门判定逻辑**：
```text
fn_issues = flow-naturalness-checker.issues
fn_critical = filter(fn_issues, severity="critical")
gt_voice_high = filter(fn_issues, rule="VOICE_PROFILE_VIOLATION", severity="high") if chapter<=3 else []
gt_info_high = filter(fn_issues, rule="INFO_OVERLOAD.SEGMENT", severity="high") if chapter<=3 else []

blockers = fn_critical + gt_voice_high + gt_info_high

if len(blockers) > 0:
    BLOCK: "自然流畅度门禁触发：{len(blockers)} 个结构性不自然问题"
    repair_context = [{
        "issue_type": issue.rule,
        "severity": issue.severity + ("→hard_block(golden_three)" if chapter<=3 and issue.severity=="high" else ""),
        "location": issue.location or "全章",
        "fix_suggestion": issue.fix_suggestion or "参考 info_budget/voice_profile 重写信息融入方式与对话"
    } for issue in blockers]
    return BLOCKED, repair_context
else:
    fn_high = filter(fn_issues, severity="high")
    if len(fn_high) > 0:
        通过（带修复建议）: "自然流畅度有 {len(fn_high)} 个 high 问题，转 polish Layer 9c/9d 修复"
    else:
        通过: "自然流畅度检查通过"
```
