...
- 默认章节字数目标：起点 2200-3000（硬下限 2200 字），番茄 1500-2000（硬下限 1500 字）。用户或大纲明确覆盖时从其约定。
...
writer-agent 产出草稿后（Step 2A）、polish 之前（Step 4），规则检查章末 100 字是否包含钩子信号：

- **钩子信号**（至少命中 1 条）：
  - 悬念句（以 `？` 结尾且非反问）
  - 转折预告（"但/可/然而/却/不料/谁知/没想到/就在这时/突然/忽然/马上/下一刻" + 新信息）
  - 情绪高点（"震惊/怒吼/跪/泪/笑" 等强情绪词）
- **不通过** → 退回 writer-agent 重写章末段落（只重写最后 150 字），不重跑全章。
- **起点不做此检查**。
...
- 字数硬区间来自创作执行包 `target_words_min` / `target_words_max`（由 `.ink/preferences.json` 的 `pacing.chapter_words` ±500 推导，默认 `(2200, 5000)`）；硬下限 2200 任何情况不得低于此值，硬上限**任何情况不得超越**——不存在章节类型级、大纲标签级、白名单级等任何 LLM 自行判定的豁免路径（US-005）。若需更高字数必须由用户在 `preferences.json` 中显式提升 `pacing.chapter_words`。
- 禁止占位符正文（如 `[TODO]`、`[待补充]`）。
...
**阈值来源**（v23 US-005）：硬上限与硬下限均来自 `.ink/preferences.json` 的 `pacing.chapter_words`，推导公式 `(max(2200, chapter_words-500), chapter_words+500)`；未配置时默认 `(2200, 5000)`。推导在 `ink-auto` 脚本与 `check_word_count` 中完成，文档中出现的 `MIN_WORDS` / `MAX_WORDS_HARD` 引用该推导结果。

**判定规则**：

| 字数范围 | 判定 | 处理方式 |
|---------|------|---------|
| < MIN_WORDS（硬下限，默认 2200） | **不合格** | **必须补写**：回到 Step 2A 在现有正文基础上扩写至 ≥ MIN_WORDS。补写时只允许：展开已有场景细节、补充角色互动/反应、增加感官描写、插入无功能感官句。禁止新增大纲之外的剧情事件。**无豁免条件，任何情况不得放行低于 MIN_WORDS 的章节。** |
| MIN_WORDS 至 MAX_WORDS_HARD（默认 2200–5000） | 合格 | 直接进入下一步 |
| > MAX_WORDS_HARD（硬上限，默认 5000） | **严重超标（硬阻断）** | **必须精简**：回到 Step 2A 对现有正文做减法，压缩至 ≤ MAX_WORDS_HARD。精简时只允许：删除冗余描写、合并重复信息、压缩过渡段落。禁止删除大纲要求的关键剧情点。**任何情况不得放行超过 MAX_WORDS_HARD 的章节。** |

**无豁免条款**（v23 US-005 硬化）：
- **硬上限由 `preferences.json` 的 `pacing.chapter_words + 500` 决定，默认 5000，任何情况不得超限放行**——不设章节类型级、节拍级、大纲标签级等任何 LLM 自行判定的豁免路径（历史版本允许的按百分比上浮或按章型放行条款，自 v23 起全部删除）；
- 若需更高字数目标，必须由用户在 `preferences.json` 中显式上调 `pacing.chapter_words`（上限随之整体抬升，而非对单章豁免）；
- **2200 字硬下限永不降**：无论 `pacing.chapter_words` 配置多低，推导出的 MIN_WORDS 永远 ≥ 2200，过渡章也不例外。

**补写/精简后必须再次检测字数**，确认进入合格范围后才可继续：
- 低于 MIN_WORDS → **补写循环最多 2 轮**（下限方向零回归，与 `ink-auto` 补写重试一致）；2 轮后仍不足 MIN_WORDS 则阻断并报告。
- 高于 MAX_WORDS_HARD → **精简循环最多 3 轮**（与 `ink-auto` `SHRINK_MAX_ROUNDS=3` 对齐，US-004/US-005）；3 轮后仍超限则阻断并报告。
...
#### Step 3.5: Harness 级闸门复检（legacy 路径 + v14 兜底）
...
- **exit 1**：硬拦截。读取 JSON 输出的 `action` 字段：
  ...
  - `rerun_step3_review`（v13 US-005）：重跑 Step 3 审查（无 review data）
  - 输出拦截原因供参考
- **exit 2**：脚本异常，输出 WARNING 并继续 Step 4（不阻断）

此步骤是确定性检查，不依赖 LLM 判断。数据源（v13 US-005）：
