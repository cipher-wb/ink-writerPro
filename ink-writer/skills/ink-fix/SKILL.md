...
5. **字数守护**：正文修复后字数必须 ∈ `[MIN_WORDS, MAX_WORDS_HARD]`（默认 `[2200, 5000]`，由 `preferences.json` 的 `pacing.chapter_words ± 500` 推导）；硬下限 2200 不可降、硬上限任何情况不得超越（v23 US-005）
...
**有限度直接修复**（对最近5章中使用重复冲突模式的最新1章操作）：
1. 从报告中提取重复的冲突模式（如"实力碾压"出现3+次）
2. 读取该章正文，定位冲突解决段落
3. 微调解决方式的描述（不改胜负结果，改路径/手段），不超过 50 字
4. 安全红线：不改角色胜负结果，不改剧情走向，仅调整解决手段的叙述
...
**有限度直接修复**（对最近3章中最适合的1章操作）：
1. 在角色对话或内心活动中补入一处自然的主题呼应，不超过 30 字
2. 示例：角色在行为选择中自然体现与主题相关的态度或代价
...
5. **字数验证**：
   ...
   # 必须 ∈ [MIN_WORDS, MAX_WORDS_HARD]（默认 [2200, 5000]，preferences 驱动；硬下限 2200 不可降、硬上限无豁免）
   ```
...
2. **正文完整性验证**（若执行了正文修复）：
   - 确认所有修改的章节文件存在且字数 ∈ `[MIN_WORDS, MAX_WORDS_HARD]`（默认 `[2200, 5000]`）
   - 运行编码校验确认无乱码
...
报告: {report_path}
自动修复: {N} 项
  - [auto] 第{ch}章: {问题描述} → 已修复
约束注入: {K} 项
  - [constraint] {描述} → 约束已写入
需人工介入: {M} 项
  - [manual] 第{ch}章: {问题描述} → 修改范围超安全红线，建议人工处理
Git: committed {hash}
INK_FIX_DONE
```
...
4. **字数硬区间**：修复后任何章节必须 ∈ `[MIN_WORDS, MAX_WORDS_HARD]`（默认 `[2200, 5000]`，preferences 驱动）——硬下限 2200 不可降、硬上限无豁免路径（v23 US-005）
