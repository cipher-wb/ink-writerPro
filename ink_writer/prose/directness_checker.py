...
def _resolve_scene_bucket(
    ...
    """根据 scene_mode/chapter_no 选 YAML bucket 的 thresholds。

    优先级：scene_mode 显式 → golden_three for ch1-3 → fallback default。
...
def is_activated(scene_mode: str | None, chapter_no: int) -> bool:
    """写作场景是否进入直白模式。

    规则：
      1. 显式 scene_mode ∈ ACTIVATION_SCENE_MODES → True
      2. scene_mode 缺省且 chapter_no ∈ [1,3] → True（等同 golden_three）
      3. 其他 → False
    """
    if scene_mode is not None:
        return scene_mode in ACTIVATION_SCENE_MODES
    return 1 <= chapter_no <= 3
...
def run_directness_check(
    ...
    *,
    ...
    参数:
      chapter_text: 章节正文
      chapter_no: 章节号（用于 golden_three 激活）
      scene_mode: 场景模式；None 时仅按 chapter_no 判定
      thresholds: 完整 YAML 结构（已 load_thresholds）或 None → 自动加载
      abstract_words: 抽象词表；None → 使用 prose-blacklist + seed 默认
    """
    activated = is_activated(scene_mode, chapter_no)
    if not activated:
        return DirectnessReport(
            skipped=True,
            reason=(
                f"scene_mode={scene_mode!r} chapter_no={chapter_no} 不激活直白模式"
            ),
    ...
    bucket_name, dim_thresholds = _resolve_scene_bucket(scene_mode, chapter_no, buckets)
...
def to_checker_output(report: DirectnessReport) -> dict[str, Any]:
    ...
    if report.skipped:
        return {
            ...
            "summary": f"场景无需直白检查（{report.reason}）",
        }
