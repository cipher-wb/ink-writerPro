"""Hard gate for ink-write: emotion curve check + polish retry loop."""
...
from ink_writer.emotion.fix_prompt_builder import normalize_checker_output


@dataclass
class EmotionGateAttempt:
    attempt: int
    score: float
...
class EmotionGateResult:
    ...
    final_score: float
    threshold: float
    attempts: list[EmotionGateAttempt] = field(default_factory=list)
    blocked_path: str | None = None
    final_text: str | None = None


def _setup_logger(chapter_no: int, project_root: str) -> logging.Logger:
    log_dir = os.path.join(project_root, "logs", "emotion-curve")
    os.makedirs(log_dir, exist_ok=True)
    log_path = os.path.join(log_dir, f"chapter_{chapter_no}.log")

    logger = logging.getLogger(f"emotion-gate-ch{chapter_no}")
...
def _write_emotion_blocked(
    ...
    blocked_path = os.path.join(chapter_dir, "emotion_blocked.md")

    lines = [
        f"# 第{chapter_no}章 — 情绪曲线门禁阻断",
        "",
...
def run_emotion_gate(
    ...
    *,
    ...
    logger.info(
        "开始情绪曲线门禁检查 chapter=%d threshold=%.1f max_retries=%d",
        chapter_no, threshold, max_retries,
    )

    current_text = chapter_text
    attempts: list[EmotionGateAttempt] = []
    violations: list[dict] = []
    fix_prompt = ""
    score: float = 0.0
    ...
    logger.warning("情绪曲线门禁 %d 次重试均未通过，章节被阻断", max_retries)
    blocked_path = _write_emotion_blocked(
