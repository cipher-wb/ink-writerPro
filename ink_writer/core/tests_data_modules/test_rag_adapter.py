...
import ink_writer.core.context.rag_adapter as rag_module
from ink_writer.core.context.rag_adapter import RAGAdapter
from ink_writer.core.infra.config import DataModulesConfig
from ink_writer.core.index.index_manager import EntityMeta, RelationshipMeta
...
async def test_search_respects_chapter_filter_across_strategies(tmp_path, monkeypatch):
    ...
    await adapter.store_chunks(
        [
            {"chapter": 1, "scene_index": 1, "content": "前文线索，秘宝尚未被发现"},
...
def test_init_db_migrates_legacy_vectors_schema(tmp_path, monkeypatch):
    ...
    backup_dir = cfg.ink_dir / "backups"
