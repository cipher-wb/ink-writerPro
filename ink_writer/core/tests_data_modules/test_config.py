...
from ink_writer.core.infra import config as config_module
from ink_writer.core.infra.config import DataModulesConfig, get_config, set_project_root


def test_config_paths_and_defaults(tmp_path):
    cfg = DataModulesConfig.from_project_root(tmp_path)
    assert cfg.project_root == tmp_path
    assert cfg.ink_dir.name == ".ink"
