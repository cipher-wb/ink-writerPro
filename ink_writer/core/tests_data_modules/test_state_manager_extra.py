...
def test_process_chapter_result_persists_narrative_memory(temp_project):
    manager = StateManager(temp_project)
    manager.add_entity(EntityState(id="xiaoyan", name="萧炎", type="角色", tier="核心"))

    result = {
        "entities_appeared": [{"id": "xiaoyan", "type": "角色", "mentions": ["萧炎"], "confidence": 1.0}],
        "chapter_meta": {"hook": {"type": "悬念钩", "content": "戒指忽然发热", "strength": "strong"}},
        "chapter_memory_card": {
            "summary": "萧炎发现戒指异常，药老身份再度成谜。",
            "goal": "查明戒指异动",
            "conflict": "药老暂时不肯明说",
            "result": "线索被进一步放大",
            "next_chapter_bridge": "必须接住戒指异动和药老隐瞒",
            "unresolved_questions": ["药老为何沉默"],
            "key_facts": ["戒指出现灼热反应"],
            "involved_entities": ["xiaoyan"],
        },
        "timeline_anchor": {
            "anchor_time": "深夜",
            "previous_time_delta": "距上章不足一刻钟",
            "to_location": "萧炎房间",
        },
        "plot_thread_updates": [
            {
                "thread_id": "ring-secret",
                "title": "戒指秘密",
                "content": "戒指忽然发热，疑似牵动药老往事",
                "status": "active",
                "priority": 95,
                "related_entities": ["xiaoyan"],
                "target_payoff_chapter": 6,
            }
        ],
        "reading_power": {
            "hook_type": "悬念钩",
    ...
    manager.process_chapter_result(3, result)
    manager.save_state()

    idx = IndexManager(temp_project)
    memory_card = idx.get_chapter_memory_card(3)
    assert memory_card["goal"] == "查明戒指异动"

    timeline = idx.get_timeline_anchor(3)
    assert timeline["to_location"] == "萧炎房间"

    threads = idx.get_active_plot_threads(limit=5)
    assert any(row["thread_id"] == "ring-secret" for row in threads)

    reading_power = idx.get_chapter_reading_power(3)
    assert reading_power["hook_type"] == "悬念钩"

    facts = idx.get_candidate_facts(chapter=3)
    assert any(row["fact_key"] == "ring-old-enemy" for row in facts)
...
def test_state_manager_cli_commands(temp_project, monkeypatch, capsys):
    ...
    def run_cli(args):
        ...
        from ink_writer.core.state import state_manager as sm
