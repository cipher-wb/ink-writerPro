...
class StateManager:
    ...
    def _ensure_state_schema(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(state.get("latest_warnings"), list):
            state["latest_warnings"] = []
    ...
    def save_state(self):
        ...
        has_pending = any(
            [
                ...
                self._pending_plot_threads is not None,
                self._pending_latest_warnings,
        ...
        try:
            with lock:
                ...
                self._pending_plot_threads = None
                self._pending_latest_warnings.clear()
    ...
    def _sync_pending_patches_to_sqlite(self, processed_appearances: set = None) -> bool:
        ...
        try:
            from ink_writer.core.state.sql_state_manager import EntityData
            from ink_writer.core.index.index_manager import EntityMeta
    ...
    def _normalize_plot_thread_item_for_state(
        self,
        chapter: int,
        raw_item: Dict[str, Any],
        index: int = 0,
    ) -> Dict[str, Any]:
        """将线程更新标准化为 state.json 兼容镜像。"""
        if not isinstance(raw_item, dict):
            return {}

        content = str(raw_item.get("content") or raw_item.get("title") or "").strip()
        if not content:
            content = f"第{chapter}章线程{index or 1}"

        raw_priority = raw_item.get("priority", raw_item.get("urgency", 50))
        try:
        ...
        if not raw_tier:
            raw_tier = "核心" if priority >= 80 else ("装饰" if priority < 35 else "支线")

        planted_chapter = raw_item.get("planted_chapter") or raw_item.get("chapter") or chapter
        target_chapter = raw_item.get("target_payoff_chapter") or raw_item.get("target_chapter") or 0
        resolved_chapter = raw_item.get("resolved_chapter") or 0
        ...
        try:
            ...
            target_chapter = 0
        try:
            resolved_chapter = int(resolved_chapter)
        except (TypeError, ValueError):
            resolved_chapter = 0

        return {
            "thread_id": str(raw_item.get("thread_id") or raw_item.get("id") or f"ch{chapter}_thread_{index or 1}"),
            "title": str(raw_item.get("title") or content[:24]),
            "content": content,
            "type": str(raw_item.get("thread_type") or raw_item.get("type") or "foreshadowing"),
            "status": normalize_foreshadowing_status(raw_item.get("status")),
            "tier": normalize_foreshadowing_tier(raw_tier),
            "urgency": priority,
            "priority": priority,
            "planted_chapter": planted_chapter,
            "chapter": planted_chapter,
            "last_touched_chapter": chapter,
            "target_chapter": target_chapter,
            "resolved_chapter": resolved_chapter,
            "related_entities": list(raw_item.get("related_entities", []) or []),
            "notes": str(raw_item.get("notes", "")),
            "confidence": raw_item.get("confidence", 1.0),
        }

    def _update_plot_threads_mirror(self, chapter: int, plot_thread_updates: Any) -> None:
        """更新 state.json 中的 plot_threads 镜像。"""
        current = deepcopy(self._state.get("plot_threads", {}))
        if not isinstance(current, dict):
            current = {}

        raw_foreshadowing = current.get("foreshadowing")
        if not isinstance(raw_foreshadowing, list):
            raw_foreshadowing = []
        ...
        for item in raw_foreshadowing:
            if not isinstance(item, dict):
                continue
            key = str(item.get("thread_id") or item.get("content") or item.get("title") or "").strip()
            if key:
                by_key[key] = dict(item)

        if isinstance(plot_thread_updates, list):
            for idx, raw_item in enumerate(plot_thread_updates, start=1):
                normalized = self._normalize_plot_thread_item_for_state(chapter, raw_item, index=idx)
                if not normalized:
                    continue
                key = str(normalized.get("thread_id") or normalized.get("content") or "").strip()
                if not key:
                    continue
                existing = by_key.get(key, {})
                merged = {**existing, **normalized}
                if existing.get("planted_chapter") and normalized.get("planted_chapter"):
                    merged["planted_chapter"] = min(
                        int(existing.get("planted_chapter") or normalized["planted_chapter"]),
                        int(normalized["planted_chapter"]),
                    )
                by_key[key] = merged
        ...
        foreshadowing = sorted(
            ...
            key=lambda row: (
                ...
                int(row.get("last_touched_chapter") or row.get("planted_chapter") or 0),
            ),
            reverse=True,
        )
        ...
        active_threads = [
            {
                ...
                "title": row.get("title") or row.get("content", "")[:24],
                "status": row.get("status"),
                "priority": row.get("priority", row.get("urgency", 0)),
                "last_touched_chapter": row.get("last_touched_chapter"),
                "target_chapter": row.get("target_chapter"),
            }
            for row in foreshadowing
            if normalize_foreshadowing_status(row.get("status")) != "已回收"
        ]

        current["foreshadowing"] = foreshadowing
        current["active_threads"] = active_threads[:20]
        self._state["plot_threads"] = current
        self._pending_plot_threads = deepcopy(current)

    def _build_chapter_meta_payload(self, chapter: int, result: Dict[str, Any], warnings: List[str]) -> Dict[str, Any]:
        ...
        if isinstance(result.get("plot_thread_updates"), list) and result.get("plot_thread_updates"):
            payload["plot_thread_updates"] = [
                ...
                for item in result.get("plot_thread_updates", [])
                if isinstance(item, dict)
            ]
        if isinstance(result.get("scenes"), list):
            payload["scene_count"] = len(result.get("scenes", []))

        explicit_warnings = result.get("warnings") if isinstance(result.get("warnings"), list) else []
        merged_warnings: List[str] = []
        for text in [*explicit_warnings, *warnings]:
            value = str(text or "").strip()
            if value and value not in merged_warnings:
                merged_warnings.append(value)
        if merged_warnings:
            payload["warnings"] = merged_warnings

        payload["updated_at"] = datetime.now().isoformat()
        payload["chapter"] = chapter
        return normalize_chapter_meta_entry(payload)
    ...
    def process_chapter_result(self, chapter: int, result: Dict) -> List[str]:
        ...
        if isinstance(scenes, list):
            self._pending_sqlite_data["scenes"] = scenes

        if isinstance(result.get("chapter_meta"), dict):
            self._pending_sqlite_data["chapter_meta"] = dict(result.get("chapter_meta"))

        if isinstance(result.get("chapter_memory_card"), dict):
            self._pending_sqlite_data["chapter_memory_card"] = dict(result.get("chapter_memory_card"))

        if isinstance(result.get("timeline_anchor"), dict):
            self._pending_sqlite_data["timeline_anchor"] = dict(result.get("timeline_anchor"))

        if isinstance(result.get("plot_thread_updates"), list):
            self._pending_sqlite_data["plot_thread_updates"] = list(result.get("plot_thread_updates"))
            self._update_plot_threads_mirror(chapter, result.get("plot_thread_updates"))

        if isinstance(result.get("reading_power"), dict):
            self._pending_sqlite_data["reading_power"] = dict(result.get("reading_power"))
        ...
        self._state["latest_warnings"] = list(warnings[-20:])
        self._pending_latest_warnings = list(warnings[-20:])
...
def main():
    ...
    from ink_writer.core.state.schemas import validate_data_agent_output, format_validation_error, normalize_data_agent_output
