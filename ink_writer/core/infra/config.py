...
from ink_writer.core.context.context_weights import TEMPLATE_WEIGHTS_DYNAMIC_DEFAULT

def _get_user_claude_root() -> Path:
    raw = os.environ.get("INK_CLAUDE_HOME") or os.environ.get("CLAUDE_HOME")
    if raw:
        try:
...
class DataModulesConfig:
    ...
    def rag_db(self) -> Path:
        return self.ink_dir / "rag.db"

    @property
    def vector_db(self) -> Path:
        return self.ink_dir / "vectors.db"

    def ensure_dirs(self):
        self.ink_dir.mkdir(parents=True, exist_ok=True)
...
def get_config(project_root: Optional[Path] = None) -> DataModulesConfig:
    ...
    if _default_config is None:
        ...
        # - 支持 INK_PROJECT_ROOT
        # - 支持 `.claude/.ink-current-project` 指针文件
        # - 支持从当前目录/父目录寻找 `.ink/state.json`
