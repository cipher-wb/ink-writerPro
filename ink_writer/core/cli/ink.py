...
# -*- coding: utf-8 -*-
"""
ink 统一入口（面向 skills / agents 的稳定 CLI）

设计目标：
- 只有一个入口命令，避免到处拼 `python -m data_modules.xxx ...` 导致参数位置/引号/路径炸裂。
- 自动解析正确的 book project_root（包含 `.ink/state.json` 的目录）。
...
典型用法（推荐，不依赖 PYTHONPATH / 不要求 cd）：
  ...
  python "<SCRIPTS_DIR>/ink.py" use D:\\wk\\xiaoshuo\\凡人资本论
...
def _build_preflight_report(explicit_project_root: Optional[str]) -> dict:
    ...
    skill_root = plugin_root / "skills" / "ink-write"
    entry_script = scripts_dir / "ink.py"
    ...
    if project_root:
        try:
            ...
            if not embed_key:
                ...
                try:
                    from ink_writer.core.infra.api_client import EmbeddingAPIClient
...
def main() -> None:
    ...
    p_use.add_argument("project_root", help="书项目根目录（必须包含 .ink/state.json）")
    ...
    from ink_writer.core.cli.cli_args import normalize_global_project_root
    ...
    if tool == "checkpoint-level":
        from ink_writer.core.cli.checkpoint_utils import cli_checkpoint_level
        cli_checkpoint_level(args)
        raise SystemExit(0)

    if tool == "report-check":
        from ink_writer.core.cli.checkpoint_utils import cli_report_check
        cli_report_check(args)
        raise SystemExit(0)

    if tool == "disambig-check":
        args.project_root = str(project_root)
        from ink_writer.core.cli.checkpoint_utils import cli_disambig_check
