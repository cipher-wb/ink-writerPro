...
HEAD_TAIL_SPLIT_MARKER = "\n\n【……中段省略（仅用于控制提示长度，章末钩子完整保留在下方）……】\n\n"


def _excerpt_chapter_text(chapter_text: str, max_chars: int) -> str:
    """Keep the head and tail so the ending hook survives truncation.

    US-009: 旧实现 `chapter_text[:5000]` 会直接砍掉章末，编辑规则里的钩子类违规（EW-hook*）
    因此从不会被触发。改成：未超限原样返回；超限则保留 head+tail 各 max_chars//2，
    中间插入省略提示。
    """
    if len(chapter_text) <= max_chars:
        return chapter_text
    half = max_chars // 2
    head = chapter_text[:half]
    tail = chapter_text[-half:]
    return f"{head}{HEAD_TAIL_SPLIT_MARKER}{tail}"
...
def _build_user_prompt(
    ...
    *,
    ...
    excerpt = _excerpt_chapter_text(chapter_text, max_chars)
    return f"""## 章节信息
章节号: {chapter_no}
