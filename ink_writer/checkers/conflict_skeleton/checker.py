"""check_conflict_skeleton() — M3 章节级冲突骨架检查（spec §4.1 + Q6+Q7）。

输出 ConflictReport：
  - score = 0.5 * has_conflict + 0.3 * has_three_stage + 0.2 * min(count / 2, 1)
  - blocked = score < block_threshold（默认 0.60）
  - 短章节（< 500 字）直接跳过：score=0.0、blocked=False、notes="skipped_short_chapter"
  - LLM/JSON 解析失败重试 max_retries 次后仍失败 → score=0.0、blocked=True、
    notes="checker_failed"
  - cases_hit 默认空列表（由 rewrite_loop 在阻断时按 tag 注入）。
"""

from __future__ import annotations

import json
import re
from pathlib import Path
from typing import Any

from ink_writer.checkers.conflict_skeleton.models import ConflictReport

PROMPT_PATH = Path(__file__).resolve().parent / "prompts" / "check.txt"

_DEFAULT_MAX_TOKENS = 2048
_DEFAULT_MODEL = "glm-4.6"
_SHORT_CHAPTER_CHARS = 500
_DEFAULT_BLOCK_THRESHOLD = 0.60


def _load_prompt_template() -> str:
    with open(PROMPT_PATH, encoding="utf-8") as fh:
        return fh.read()


def _build_prompt(chapter_text: str) -> str:
    return _load_prompt_template().format(chapter_text=chapter_text)


def _extract_json(raw: str) -> dict[str, Any]:
    if not isinstance(raw, str):
        raise ValueError("llm response is not a string")
    text = raw.strip()
    fence = re.search(r"```(?:json)?\s*(\{.*?\})\s*```", text, re.DOTALL)
    if fence:
        text = fence.group(1)
    if not text.startswith("{"):
        m = re.search(r"\{.*\}", text, re.DOTALL)
        if not m:
            raise ValueError("no JSON object found in llm response")
        text = m.group(0)
    return json.loads(text)


def _call_llm(llm_client: Any, prompt: str, model: str) -> str:
    response = llm_client.messages.create(
        model=model,
        max_tokens=_DEFAULT_MAX_TOKENS,
        messages=[{"role": "user", "content": prompt}],
    )
    return response.content[0].text


def _compute_score(*, has_conflict: bool, has_three_stage: bool, count: int) -> float:
    count_term = min(max(count, 0) / 2.0, 1.0)
    return 0.5 * float(has_conflict) + 0.3 * float(has_three_stage) + 0.2 * count_term


def check_conflict_skeleton(
    *,
    chapter_text: str,
    book: str,  # noqa: ARG001  仅用于上下文/日志
    chapter: str,  # noqa: ARG001  仅用于上下文/日志
    llm_client: Any,
    max_retries: int = 3,
    block_threshold: float = _DEFAULT_BLOCK_THRESHOLD,
    model: str = _DEFAULT_MODEL,
) -> ConflictReport:
    """spec §4.1 实现 — 见模块 docstring。"""
    if len(chapter_text) < _SHORT_CHAPTER_CHARS:
        return ConflictReport(
            has_explicit_conflict=False,
            conflict_count=0,
            has_three_stage_structure=False,
            conflict_summaries=[],
            score=0.0,
            block_threshold=block_threshold,
            blocked=False,
            cases_hit=[],
            notes="skipped_short_chapter",
        )

    prompt = _build_prompt(chapter_text)

    parsed: dict[str, Any] | None = None
    attempts = max(1, max_retries)
    for _ in range(attempts):
        try:
            raw = _call_llm(llm_client, prompt, model)
            parsed = _extract_json(raw)
            break
        except Exception:  # noqa: BLE001  LLM/JSON 失败统一降级
            parsed = None
            continue

    if parsed is None:
        return ConflictReport(
            has_explicit_conflict=False,
            conflict_count=0,
            has_three_stage_structure=False,
            conflict_summaries=[],
            score=0.0,
            block_threshold=block_threshold,
            blocked=True,
            cases_hit=[],
            notes="checker_failed",
        )

    has_conflict = bool(parsed.get("has_explicit_conflict", False))
    try:
        conflict_count = int(parsed.get("conflict_count", 0))
    except (TypeError, ValueError):
        conflict_count = 0
    if conflict_count < 0:
        conflict_count = 0
    has_three_stage = bool(parsed.get("has_three_stage_structure", False))

    summaries: list[str] = []
    conflicts_raw = parsed.get("conflicts") or []
    if isinstance(conflicts_raw, list):
        for entry in conflicts_raw:
            if not isinstance(entry, dict):
                continue
            parts = [
                str(entry.get("friction_point", "")).strip(),
                str(entry.get("escalation", "")).strip(),
                str(entry.get("interim_resolution", "")).strip(),
            ]
            summary = " → ".join(p for p in parts if p)
            if summary:
                summaries.append(summary)

    score = _compute_score(
        has_conflict=has_conflict,
        has_three_stage=has_three_stage,
        count=conflict_count,
    )
    blocked = score < block_threshold

    notes_value = parsed.get("notes", "")
    notes = notes_value if isinstance(notes_value, str) else ""

    return ConflictReport(
        has_explicit_conflict=has_conflict,
        conflict_count=conflict_count,
        has_three_stage_structure=has_three_stage,
        conflict_summaries=summaries,
        score=score,
        block_threshold=block_threshold,
        blocked=blocked,
        cases_hit=[],
        notes=notes,
    )
